#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# bot_guncel.py - VIP Sorgu Paneli (Tüm API + Güvenlik Katmanları)
# Ek: Gelen cevaplara sahip + telegram kanal bilgisi eklenir.
# Sahip: @Keneviiz
# Yapımcı: @obezoc
# Telegram kanalları: http://T.me/KenevizApiSystem , https://t.me/BozkurtCyberTeam
...
SAHIP = "@Keneviiz"
TELEGRAM_KANAL = "http://T.me/KenevizApiSystem"
YAPIMCI = "@obezoc"
YAPIMCI_KANAL = "https://t.me/BozkurtCyberTeam"
...
APIS = {
    ...
    "anne": {"desc": "Anne Sorgulama", "url": "https://hexnox.pro/sowixfree/anne.php", "params": ["tc"]},
...
def api_proxy(api_name):
    ...
    try:
        ...
        # Eğer API JSON dönmezse KeyError/ValueError alabilir, bu yüzden güvenli parse
        try:
            data = r.json()
        except ValueError:
        ...
        if isinstance(data, dict) and "info" in data:
            data["info"] = f"Hata alırsanız {SAHIP} veya {YAPIMCI} Telegram’dan ulaşabilirsiniz."
        ...
        if isinstance(data, dict):
            ...
            data["yapimci"] = YAPIMCI
            data["yapimci_kanal"] = YAPIMCI_KANAL
    except Exception:
        data = {
            "error": "Kayıt bulunamadı",
            "info": f"Hata alırsanız {SAHIP} veya {YAPIMCI} Telegram’dan ulaşabilirsiniz.",
            "sahip": SAHIP,
            "telegram_kanal": TELEGRAM_KANAL,
            "yapimci": YAPIMCI,
            "yapimci_kanal": YAPIMCI_KANAL
        }
...
def index():
    return (
        f"<h1>VIP Sorgu Paneli Çalışıyor</h1>"
        f"<p>Sahip: {SAHIP} — Kanal: <a href='{TELEGRAM_KANAL}'>{TELEGRAM_KANAL}</a></p>"
        f"<p>Yapımcı: {YAPIMCI} — Kanal: <a href='{YAPIMCI_KANAL}'>{YAPIMCI_KANAL}</a></p>"
    )
